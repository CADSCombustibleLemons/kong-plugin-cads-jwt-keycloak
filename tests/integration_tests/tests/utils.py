...
def create_api(config, expected_response=CREATED):
    def real_decorator(func):
        def wrapper(*args, **kwargs):
            ...
            r = requests.post(
                KONG_ADMIN + "/services",
                data={"name": api_name, "url": "http://mockbin.org/headers"},
            )
            assert r.status_code == CREATED
            r = requests.post(
                KONG_ADMIN + "/services/" + api_name + "/routes",
                data={"name": api_name, "paths[]": "/" + api_name},
            )
            assert r.status_code == CREATED
            r = requests.post(
                KONG_ADMIN + "/services/" + api_name + "/plugins",
                json={"name": "jwt-keycloak", "config": config},
            )
...
def call_api(token=None, method="get", params=None, endpoint=None):
    def real_decorator(func):
        def wrapper(*args, **kwargs):
            ...
            # services = requests.get(KONG_ADMIN + "/services")
            # print("services")
            # print(services.json())
            # routes = requests.get(KONG_ADMIN + "/routes")
            # print("routes")
            # print(routes.json())
            # routes = requests.get(KONG_API + "/test")
            # print("routes")
            # print(routes.json())
...
def create_client(client_id, **kwargs):
    ...
    r = requests.post(
        KC_HOST + "/admin/realms/master/clients",
    ...
    r = requests.post(
        KC_HOST + "/admin/realms/master/clients/" + id + "/roles",
        headers={"Authorization": "Bearer " + KC_ADMIN_TOKEN},
        json={"name": "test_role"},
    )
...
def authenticate(**kwargs_outer):
    def real_decorator(func):
        def wrapper(*args, **kwargs):
            ...
            # print(client_id)
            # print(client_secret)
            client_token = get_kc_token(client_id, client_secret)
            # print(client_token)
            result = func(*args, token=client_token, **kwargs)
            return result
...
def kc_get_key(name):
    r = requests.get(
        KC_HOST
        + "/admin/realms/master/components?parent=master&type=org.keycloak.keys.KeyProvider",
        headers={"Authorization": "Bearer " + KC_ADMIN_TOKEN},
    )
...
def kc_delete_key(key):
    r = requests.delete(
        KC_HOST + "/admin/realms/master/components/" + key["id"],
        headers={"Authorization": "Bearer " + KC_ADMIN_TOKEN},
    )
...
def kc_add_key(name, priority):
    r = requests.post(
        KC_HOST + "/admin/realms/master/components",
        headers={"Authorization": "Bearer " + KC_ADMIN_TOKEN},
        json={
            "config": {
                "active": [True],
    ...
    r = requests.get(
        r.headers["Location"], headers={"Authorization": "Bearer " + KC_ADMIN_TOKEN}
    )
    assert r.status_code == 200
